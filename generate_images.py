...
def edm_sampler(
    ...
    num_steps=32, sigma_min=0.002, sigma_max=80, rho=7, guidance=1, coeffs=None,
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,
    dtype=torch.float32, randn_like=torch.randn_like,
):
    # Guided denoiser.
    if coeffs is None:  # Vanilla CFG.
        def denoise(x, t, i):
        ...
        def denoise(x, t, i):
            ...
            if guidance == 1:
                return Dx
            ref_Dx = gnet(x, t, labels).to(dtype)
            gamma_1 = torch.ones_like(Dx) * guidance
            gamma_0 = (gamma_1 - 1.) * coeffs[i].to(dtype)
            net_d_cur = (x - Dx) / t
            gnet_d_cur = (x - ref_Dx) / t
            d_cur = gamma_0 * gnet_d_cur + gamma_1 * net_d_cur
            return x - d_cur * t
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Euler step.
        d_cur = (x_hat - denoise(x_hat, t_hat, i)) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            d_prime = (x_next - denoise(x_next, t_next, i)) / t_next
...
def generate_images(
    ...
    coeffs              = None,                 # Expectation ratios for guidance.
    ...
    **sampler_kwargs,                           # Additional arguments for the sampler function.
    ...
    if coeffs is not None:
        if isinstance(coeffs, str):
            if verbose:
                dist.print0(f'Loading lookup table from {coeffs} ...')
            with dnnlib.util.open_url(coeffs, verbose=(verbose and dist.get_rank() == 0)) as f:
                coeffs = pickle.load(f).to(device)
                # By default, the last row of coeffs indicates the mean over all the other conditions.
                # Then repeat the last row for convenience.
                coeffs = torch.cat([coeffs[:-1], coeffs[-1:].repeat(net.label_dim - len(coeffs) + 1, *[1] * (coeffs.ndim - 1))], dim=0)
        score_shape = coeffs.shape[1:]
            ...
            for batch_idx, indices in enumerate(rank_batches):
                ...
                if len(r.seeds) > 0:
                    ...
                    if net.label_dim > 0:
                        ...
                        if coeffs is not None:
                            r.coeffs = torch.matmul(r.labels, coeffs.flatten(1)).view(-1, *score_shape)
                        else:
                            r.coeffs = None
                    else:
                        r.coeffs = None
                    ...
                    latents = dnnlib.util.call_func_by_name(func_name=sampler_fn, net=net, noise=r.noise,
                        labels=r.labels, gnet=gnet, coeffs=r.coeffs, randn_like=rnd.randn_like, **sampler_kwargs)
...
@click.option('--coeffs',                   help='Coefficient pickle filename', metavar='PATH|URL',                 type=str, default=None)
